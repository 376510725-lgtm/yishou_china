    ...
    async configureServer(server) {
      server.middlewares.use(async (req, res, next) => {
        try {
          ...
          if (route.assetPath) {
            // Pass module script requests to Vite for proper transformation
            // (e.g. import logoImg from './logo.png' needs Vite to convert it to a JS module)
            if (getHeaderValue(req.headers?.['sec-fetch-dest']).toLowerCase() === 'script') {
              next();
              return;
            }
            // Also pass requests with ?import or ?url query params to Vite for transformation
            // Vite handles these as URL modules, returning JS that exports the URL string
            const urlParams = new URLSearchParams(req.url?.split('?')[1] || '');
            if (urlParams.has('import') || urlParams.has('url')) {
              next();
              return;
            }
