...
// Types
// ─────────────────────────────────────────────
type AppScreen = 'login' | 'main' | 'service' | 'service-zhuyu' | 'service-zhujie' | 'service-zhuyi' | 'service-zhuju' | 'service-zhuxing' | 'service-liaoyang' | 'service-activity' | 'service-yanglao' | 'service-ganchang' | 'service-juhui' | 'service-bisai' | 'health-yangsheng' | 'calendar-perpetual' | 'chat' | 'room-chat' | 'post-detail' | 'video-feed' | 'user-profile' | 'my-dynamics' | 'my-favorites' | 'settings' | 'my-profile' | 'contacts' | 'friend-requests' | 'notify-likes' | 'notify-thumbs' | 'notify-comments' | 'notify-fans' | 'create-select' | 'create-image' | 'create-video' | 'activity-detail' | 'competition-detail' | 'comment-detail' | 'address-management' | 'change-password' | 'bayu-search' | 'points-mall' | 'points-detail' | 'points-records' | 'order-detail' | 'mall-favorites' | 'mall-address' | 'account-cancellation' | 'notification-settings' | 'privacy-settings' | 'check-update' | 'about-us' | 'friend-detail' | 'my-detail' | 'follow-list' | 'my-wallet' | 'pindan-home' | 'pindan-activity' | 'pindan-product' | 'pindan-confirm' | 'pindan-orders' | 'pindan-order-detail' | 'partner-join' | 'influencer-join' | 'medication-home' | 'medication-add' | 'medication-calendar' | 'medication-detail';
...
interface RoomMessage {
  ...
  image?: string;
  time?: string;
  system?: string; // 系统消息文字
}
const ROOM_MEMBERS: RoomMember[] = [
  { id: 101, name: '王阿姨', avatarSeed: 'room_member1', color: '#FF6B6B' },
  { id: 102, name: '李大爷', avatarSeed: 'room_member2', color: '#4ECDC4' },
  { id: 103, name: '张姐',   avatarSeed: 'room_member3', color: '#FF8A5C' },
  { id: 104, name: '刘叔',   avatarSeed: 'room_member4', color: '#6C5CE7' },
  { id: 105, name: '赵老师', avatarSeed: 'room_member5', color: '#FDCB6E' },
  { id: 106, name: '孙阿姨', avatarSeed: 'room_member6', color: '#00CEC9' },
  { id: 107, name: '陈大哥', avatarSeed: 'room_member7', color: '#E17055' },
  { id: 108, name: '周姐',   avatarSeed: 'room_member8', color: '#0984E3' },
];
const M = (id: number, name: string, avatarSeed: string, color: string): RoomMember => ({ id, name, avatarSeed, color });
const SYS = (id: number, text: string, time?: string): RoomMessage => ({ id, type: 'text' as MsgType, senderName: '', senderAvatar: '', senderColor: '', isMe: false, system: text, time });
const MSG = (id: number, m: RoomMember, content: string, time?: string): RoomMessage => ({ id, type: 'text' as MsgType, senderName: m.name, senderAvatar: m.avatarSeed, senderColor: m.color, isMe: false, content, time });
const MSG_ME = (id: number, content: string, time?: string): RoomMessage => ({ id, type: 'text' as MsgType, senderName: '', senderAvatar: '', senderColor: '', isMe: true, content, time });
const IMG = (id: number, m: RoomMember, image: string, time?: string): RoomMessage => ({ id, type: 'image' as MsgType, senderName: m.name, senderAvatar: m.avatarSeed, senderColor: m.color, isMe: false, image, time });
const IMG_ME = (id: number, image: string, time?: string): RoomMessage => ({ id, type: 'image' as MsgType, senderName: '', senderAvatar: '', senderColor: '', isMe: true, image, time });
const VOICE = (id: number, m: RoomMember, duration: number, time?: string): RoomMessage => ({ id, type: 'voice' as MsgType, senderName: m.name, senderAvatar: m.avatarSeed, senderColor: m.color, isMe: false, duration, time });

const [WA, LYD, ZJ, LS, ZLS, SAY, CDG, ZJ2] = ROOM_MEMBERS;

// 每个圈子的初始消息
const ROOM_MESSAGES_MAP: Record<string, RoomMessage[]> = {
  // 广场舞聊天室
  'r1': [
    SYS(1, '你已加入「广场舞聊天室」，和大家打个招呼吧～', '今天 08:30'),
    MSG(2, WA, '早上好啊各位！今天天气真不错，适合跳舞💃'),
    MSG(3, LYD, '王阿姨早！我刚从公园回来，练了一段新的舞步'),
    MSG_ME(4, '大家早上好！我是新来的，请多关照😊', '今天 08:32'),
    MSG(5, ZLS, '欢迎新朋友！@张大爷 您喜欢跳哪种风格的广场舞？'),
    MSG_ME(6, '我喜欢民族舞和健身操，还在学呢'),
    MSG(7, WA, '张大爷别谦虚，慢慢学，我们这好多都是零基础起步的'),
    IMG(8, ZLS, 'https://picsum.photos/seed/dance_group1/320/240', '今天 08:40'),
    MSG(9, ZLS, '这是上次我们在社区广场活动的照片，可热闹了！'),
    MSG(10, LYD, '哈哈，我在里面呢！跳的是《最炫民族风》'),
    MSG_ME(11, '这活动真不错，下次我也要参加！什么时候有活动？'),
    MSG(12, WA, '每周二四六早上7点，社区广场集合。这周六就有！'),
    MSG(13, SAY, '周六我也来，好久没跳了，音乐一响就想动'),
    MSG_ME(14, '好的，周六一定到！😄'),
  ],
  // 太极拳交流群
  'r2': [
    SYS(1, '你已加入「太极拳交流群」，一起修身养性～', '今天 07:15'),
    MSG(2, LYD, '各位拳友早上好！今天练习了陈式太极拳老架一路，感觉气息通畅'),
    MSG(3, CDG, '李大爷练得真好！我最近在学42式，第二节的转身有点难度'),
    MSG_ME(4, '大家早！我刚开始学太极，有什么适合新手的建议吗？'),
    MSG(5, LYD, '新手建议从简化24式开始，动作幅度小，容易掌握', '今天 07:20'),
    MSG(6, ZJ2, '对对，我也是从24式入门的。关键是要放松、呼吸配合'),
    MSG(7, CDG, '张大爷如果感兴趣，可以每天早上6:30来滨江公园，我们有个晨练小组'),
    MSG_ME(8, '太好了，滨江公园离我家很近，明天就来！'),
    MSG(9, LYD, '欢迎！记得穿宽松的衣服，带瓶温水'),
  ],
  // 川菜美食分享
  'r3': [
    SYS(1, '你已加入「川菜美食分享」，聊聊你最爱的美食吧～', '今天 11:45'),
    MSG(2, ZJ, '中午好！今天做了个水煮鱼，麻辣鲜香，太过瘾了！🌶️'),
    IMG(3, ZJ, 'https://picsum.photos/seed/sichuan_fish/320/240', '今天 11:48'),
    MSG(4, SAY, '哇张姐，看得我流口水！能不能教教我们怎么做？'),
    MSG(5, ZJ, '没问题啊！先用花椒和干辣椒爆香锅底，鱼片要切薄一点，腌制的时候放姜丝料酒'),
    MSG_ME(6, '我也爱吃川菜，但年纪大了不能吃太辣😅', '今天 11:52'),
    MSG(7, LS, '可以少放点辣椒，多用花椒提麻味，这样既香又不伤胃'),
    MSG(8, ZJ, '刘叔说得对，我们老年人做川菜要适当改良，少油少盐少辣，味道一样好'),
    MSG_ME(9, '学到了！周末在家试试改良版水煮鱼'),
  ],
  // 旅游摄影圈
  'r4': [
    SYS(1, '你已加入「旅游摄影圈」，分享旅途美景～', '今天 14:20'),
    IMG(2, WA, 'https://picsum.photos/seed/travel_guilin/320/240', '今天 14:22'),
    MSG(3, WA, '分享一下上个月去桂林拍的照片，山水如画⛰️'),
    MSG(4, ZLS, '太美了！王阿姨拍照水平越来越高了，用什么相机拍的？'),
    MSG(5, WA, '就是手机拍的，华为的。关键是光线和构图'),
    MSG_ME(6, '我也喜欢旅游拍照，下次可以组织个摄影采风活动吗？'),
    MSG(7, CDG, '好主意！最近巫山的红叶快红了，十月底可以去拍'),
    MSG(8, ZLS, '支持！我来做行程策划，报名接龙开起来'),
    MSG_ME(9, '报名！算我一个 🙋'),
  ],
  // 养生保健交流
  'r5': [
    SYS(1, '你已加入「养生保健交流」，健康是福～', '今天 06:50'),
    MSG(2, LS, '各位早！分享一个秋季养生方子：银耳莲子羹，润肺止咳效果好'),
    MSG(3, SAY, '我最近睡眠不好，刘叔有没有什么好办法？'),
    MSG(4, LS, '孙阿姨，睡前用热水泡脚15分钟，按摩涌泉穴，效果不错'),
    MSG_ME(5, '我有个小妙招：晚上喝杯温牛奶，加点蜂蜜，助眠效果很好👍'),
    MSG(6, ZJ, '大家会养生，真好啊！我每天早上打八段锦，坚持半年了，腰不酸了'),
    MSG(7, LS, '八段锦确实好，动作温和不伤关节，特别适合我们这个年纪'),
    MSG(8, SAY, '那我从今天开始试试泡脚+喝牛奶，谢谢大家！'),
  ],
  // 麻将棋牌娱乐
  'r6': [
    SYS(1, '你已加入「麻将棋牌娱乐」，三缺一的时候喊一声～', '今天 15:10'),
    MSG(2, CDG, '下午茶后搓一盘不？群里凑一桌 🌟'),
    MSG(3, LS, '来！在哪？老地方社区活动室？'),
    MSG(4, WA, '算我一个，刚好在家没事'),
    MSG_ME(5, '我也来！不过我是新手，水平有限哈'),
    MSG(6, CDG, '没事，我们都是娱乐为主，不赌钱的哈哈', '今天 15:13'),
    MSG(7, LS, '那就活动室见，2点40到'),
    MSG(8, CDG, '好！三缺一缺的张大爷到了，满员开打！😄'),
  ],
  // 红歌合唱团
  'r7': [
    SYS(1, '你已加入「红歌合唱团」，歌声嘹亮～', '今天 09:05'),
    MSG(2, ZLS, '各位团员早上好！下周五社区文艺汇演，我们团报了三首歌'),
    MSG(3, WA, '太好了！哪三首？我得赶紧练练'),
    MSG(4, ZLS, '《我和我的祖国》《南泥湾》《打靶归来》'),
    MSG_ME(5, '收到！《我和我的祖国》我会唱，其他两首还需要练习'),
    MSG(6, SAY, '没问题，这周四下午3点排练，在社区二楼多功能厅'),
    MSG(7, WA, '我把曲谱发群里，大家先熟悉一下'),
    MSG(8, ZLS, '对了，演出服统一穿白色上衣+深色裤子，男士深蓝、女士黑色'),
    MSG_ME(9, '收到！周四准时到 🎵'),
  ],
  // 书法绘画交流
  'r8': [
    SYS(1, '你已加入「书法绘画交流」，笔墨传情～', '今天 10:30'),
    MSG(2, ZJ2, '早上好！刚完成一幅隶书作品，请大家指教'),
    IMG(3, ZJ2, 'https://picsum.photos/seed/calligraphy_work/320/240', '今天 10:35'),
    MSG(4, LS, '周姐的字功力深厚！这隶书写的真稳'),
    MSG(5, ZLS, '确实，笔画有力又有韵味。周姐练了多久了？'),
    MSG(6, ZJ2, '谢谢夸奖！练了二十多年了，退休后更是每天写两小时'),
    MSG_ME(7, '好厉害！我刚开始学国画，画的牡丹还像个萝卜😂'),
    MSG(8, CDG, '哈哈张大爷别急，慢慢来。可以带上作品来社区书画社，大家互相交流'),
    MSG(9, ZJ2, '对，每周三下午书画社有活动，欢迎来参加！'),
    MSG_ME(10, '好的，周三我带上我的"萝卜牡丹"来 😄'),
  ],
};
...
const RoomChatScreen: React.FC<{
...
}> = ({ roomName, roomColor, onlineCount, memberCount, messages, onBack, onSend }) => {
  ...
  return (
    <>
      {/* 顶部导航栏 */}
      <div className="flex items-center gap-2 px-3 flex-shrink-0" style={{ position: 'relative', background: 'white', height: 54, borderBottom: '1px solid #F0F0F0' }}>
        <button onClick={onBack} className="flex items-center justify-center rounded-xl border-0 bg-transparent cursor-pointer p-1 flex-shrink-0">
          <ArrowLeftOutlined style={{ fontSize: 20, color: '#1A1A1A' }} />
        </button>
        <div style={{ position: 'absolute', left: 0, right: 0, top: 0, bottom: 0, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', pointerEvents: 'none' }}>
          <div style={{ fontSize: 17, fontWeight: 700, color: '#1A1A1A', lineHeight: 1.3 }}>{roomName}</div>
          <div style={{ fontSize: 11, color: '#999', lineHeight: 1.3 }}>{onlineCount}人在线 · {memberCount}名成员</div>
        </div>
        <button
          onClick={() => setShowMembers(true)}
          className="flex items-center justify-center rounded-xl border-0 bg-transparent cursor-pointer p-2 flex-shrink-0"
          style={{ marginLeft: 'auto' }}
        >
          <TeamOutlined style={{ fontSize: 22, color: '#1A1A1A' }} />
        </button>
      </div>

      {/* 消息列表 */}
      <div ref={listRef} className="flex-1 overflow-y-auto" style={{ padding: '12px 16px', background: CHAT_BG, display: 'flex', flexDirection: 'column', gap: 10 }}>
        {messages.map(msg => (
          <React.Fragment key={msg.id}>
            {msg.time && <TimeDivider time={msg.time} />}
            {msg.system ? (
              <div className="flex items-center justify-center py-1">
                <span className="px-3 py-1 rounded-xl" style={{ fontSize: 12, color: '#AAA', background: 'rgba(0,0,0,0.06)' }}>{msg.system}</span>
              </div>
            ) : msg.isMe ? (
              /* 我的消息 - 右对齐 */
              <div className="flex items-end gap-2 flex-row-reverse">
                <div className="rounded-full flex-shrink-0 flex items-center justify-center text-white font-bold" style={{ width: 36, height: 36, background: PRIMARY, fontSize: 13 }}>我</div>
                {msg.type === 'text' && (
                  <div style={{ maxWidth: 220, padding: '10px 14px', background: PRIMARY, color: 'white', borderRadius: '18px 18px 4px 18px', fontSize: 16, lineHeight: 1.6, wordBreak: 'break-word' }}>{msg.content}</div>
                )}
                {msg.type === 'voice' && (
                  <div className="flex items-center gap-2 rounded-xl px-3 cursor-pointer" style={{ height: 40, background: PRIMARY, color: 'white' }}>
                    <SoundOutlined style={{ fontSize: 16 }} />
                    <span style={{ fontSize: 14, fontWeight: 500 }}>{msg.duration}"</span>
                  </div>
                )}
                {msg.type === 'image' && (
                  <img src={msg.image} alt="" style={{ width: 160, height: 120, borderRadius: 12, objectFit: 'cover' }} />
                )}
              </div>
            ) : (
              /* 他人的消息 - 左对齐 */
              <div className="flex items-end gap-2">
                <div className="rounded-full flex-shrink-0 flex items-center justify-center text-white font-bold"
                  style={{ width: 36, height: 36, background: msg.senderColor, fontSize: 13 }}>
                  {msg.senderName.slice(0, 2)}
                </div>
                <div style={{ display: 'flex', flexDirection: 'column', gap: 3 }}>
                  <span style={{ fontSize: 12, color: '#999', paddingLeft: 4 }}>{msg.senderName}</span>
                  {msg.type === 'text' && (
                    <div style={{ maxWidth: 220, padding: '10px 14px', background: 'white', color: '#1A1A1A', borderRadius: '18px 18px 18px 4px', fontSize: 16, lineHeight: 1.6, wordBreak: 'break-word', boxShadow: '0 1px 3px rgba(0,0,0,0.06)' }}>{msg.content}</div>
                  )}
                  {msg.type === 'voice' && (
                    <div className="flex items-center gap-2 rounded-xl px-3 cursor-pointer" style={{ height: 40, background: 'white', boxShadow: '0 1px 3px rgba(0,0,0,0.06)' }}>
                      <PlayCircleOutlined style={{ fontSize: 20, color: '#1A1A1A' }} />
                      <SoundOutlined style={{ fontSize: 14, opacity: 0.6 }} />
                      <span style={{ fontSize: 14, fontWeight: 500 }}>{msg.duration}"</span>
                    </div>
                  )}
                  {msg.type === 'image' && (
                    <img src={msg.image} alt="" style={{ width: 160, height: 120, borderRadius: 12, objectFit: 'cover', display: 'block' }} />
                  )}
                </div>
              </div>
            )}
      ...
      {/* 输入栏 */}
      <div className="flex items-center gap-2 px-3 py-2 flex-shrink-0" style={{ background: 'white', borderTop: '1px solid #E8E8E8', paddingBottom: 16 }}>
        <button onClick={() => setVoiceMode(v => !v)} className="flex items-center justify-center rounded-xl border-0 bg-transparent cursor-pointer flex-shrink-0" style={{ width: 40, height: 40, color: '#666' }}>
          {voiceMode ? <span style={{ fontSize: 20, lineHeight: 1 }}>⌨</span> : <AudioOutlined style={{ fontSize: 22 }} />}
        </button>
        {voiceMode ? (
          <div className="flex-1 rounded-2xl font-semibold flex items-center justify-center select-none"
            style={{ height: 44, fontSize: 16, background: '#F5F5F5', color: '#888' }}>
            按住 说话
          </div>
        ) : (
          <div className="flex-1 flex items-center rounded-2xl px-4" style={{ height: 44, background: '#F5F5F5', border: '1.5px solid #E8E8E8' }}>
            <input type="text" value={text} onChange={e => setText(e.target.value)} onKeyDown={e => e.key === 'Enter' && handleSend()}
              placeholder="说点什么..." className="flex-1 border-0 outline-none bg-transparent" style={{ fontSize: 16, color: '#1A1A1A' }} />
          </div>
        )}
        {!voiceMode && (
          <button className="flex items-center justify-center rounded-xl border-0 bg-transparent cursor-pointer flex-shrink-0" style={{ width: 40, height: 40, color: '#888' }}>
      ...
      {/* 成员列表面板 */}
      {showMembers && (
        <div className="flex flex-col justify-end" style={{ position: 'absolute', top: 0, left: 0, right: 0, bottom: 0, zIndex: 250, background: 'rgba(0,0,0,0.4)' }} onClick={() => setShowMembers(false)}>
          <div className="w-full rounded-t-3xl flex flex-col" style={{ height: '55%', background: 'white' }} onClick={e => e.stopPropagation()}>
            <div className="flex items-center justify-between px-5 py-4 border-b" style={{ borderColor: '#F0F0F0' }}>
              <span style={{ fontSize: 18, fontWeight: 700 }}>圈子成员 ({memberCount})</span>
              <button onClick={() => setShowMembers(false)} className="flex items-center justify-center rounded-full border-0 bg-transparent cursor-pointer" style={{ width: 32, height: 32 }}><CloseOutlined style={{ fontSize: 18, color: '#999' }} /></button>
            </div>
            <div className="flex-1 overflow-y-auto px-4 py-3" style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
              {/* 当前用户 */}
              <div className="flex items-center gap-3 rounded-xl p-3" style={{ background: `${PRIMARY}10` }}>
                <div className="rounded-full flex-shrink-0 flex items-center justify-center text-white font-bold" style={{ width: 44, height: 44, background: PRIMARY, fontSize: 16 }}>我</div>
                <div style={{ flex: 1 }}>
                  <div style={{ fontSize: 15, fontWeight: 600, color: '#1A1A1A' }}>张大爷</div>
                  <div style={{ fontSize: 12, color: '#999' }}>群主</div>
                </div>
                <span className="px-2 py-0.5 rounded-full" style={{ fontSize: 11, color: '#52C41A', background: '#F6FFED', fontWeight: 500 }}>在线</span>
              </div>
              {ROOM_MEMBERS.map(m => (
                <div key={m.id} className="flex items-center gap-3 rounded-xl p-3" style={{ background: '#FAFAFA' }}>
                  <div className="rounded-full flex-shrink-0 flex items-center justify-center text-white font-bold" style={{ width: 44, height: 44, background: m.color, fontSize: 16 }}>{m.name.slice(0, 2)}</div>
                  <div style={{ flex: 1 }}>
...
const BayuTab: React.FC<{
...
}> = ({ onOpenPost, onOpenVideo, onOpenComment, onOpenUserProfile, onCreatePost, onNavigate, yishouSubTab, setYishouSubTab, onShare, userPoints, onSetUserPoints, onEnterRoom }) => {
  ...
  const [showCreateRoom, setShowCreateRoom] = useState(false); // 创建聊天室弹窗
  const [newRoomForm, setNewRoomForm] = useState({ name: '', description: '', tag: '' });
  const [joinRoomId, setJoinRoomId] = useState<string | null>(null); // 正在加入的房间ID
  const ROOM_TAGS = ['舞蹈', '运动', '美食', '旅行', '养生', '娱乐', '音乐', '艺术', '摄影', '棋牌', '读书', '手工'];
      ...
      {sub === 'chatroom' && (
        <div className="flex flex-col" style={{ background: PAGE_BG, height: '100%', position: 'relative' }}>
          ...
          <div className="flex flex-col gap-3 p-4" style={{ paddingBottom: 100 }}>
            {/* 新建圈子入口 */}
            <div
              className="rounded-2xl flex items-center justify-between cursor-pointer active:scale-[0.98] transition-transform"
              style={{
                background: `linear-gradient(135deg, ${PRIMARY_BG}, #FFF0E0)`,
                padding: '14px 18px',
            ...
            >
              <div className="flex items-center gap-3">
                <div
                  ...
                  style={{ width: 44, height: 44, background: `${PRIMARY}18` }}
                >
                  <PlusOutlined style={{ fontSize: 20, color: PRIMARY }} />
                </div>
                <div>
                  <div style={{ fontSize: 15, fontWeight: 700, color: '#1A1A1A' }}>创建新圈子</div>
            ...
            {chatRooms.map(room => {
              return (
                <div
                  key={room.id}
                  className="rounded-2xl transition-all active:scale-[0.98]"
                  style={{
                    background: 'white',
                    padding: 16,
                ...
                >
                  <div className="flex items-start gap-3">
                    ...
                    >
                      <span style={{ fontSize: 22, color: room.color, fontWeight: 700 }}>
                        {room.name.charAt(0)}
                      </span>
                    </div>
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center justify-between">
                        <div style={{ fontSize: 16, fontWeight: 700, color: '#1A1A1A' }}>{room.name}</div>
                        <div style={{ fontSize: 11, color: '#999', flexShrink: 0, marginLeft: 8 }}>
                      ...
                      <div className="flex items-center gap-2 mt-1">
                        <span
                          ...
                          style={{ fontSize: 11, color: room.color, background: `${room.color}15`, fontWeight: 600 }}
                        >
                          {room.tag}
                        </span>
                        <span style={{ fontSize: 12, color: '#999' }}>{room.memberCount} 人</span>
                        <span style={{ fontSize: 12, color: '#52C41A' }}>● {room.onlineCount} 在线</span>
                      </div>
                    ...
                    {/* 进入聊天按钮 */}
                    <button
                      onClick={(e) => { e.stopPropagation(); onEnterRoom(room.id); }}
                      className="flex-shrink-0 flex items-center justify-center rounded-full border-0 cursor-pointer active:scale-90 transition-all self-center"
                      style={{
                        width: 72,
                        height: 32,
                        background: PRIMARY,
          ...
          {showCreateRoom && (
            <div
              ...
              style={{ position: 'absolute', top: 0, left: 0, right: 0, bottom: 0, zIndex: 200, background: 'rgba(0,0,0,0.5)', overflow: 'hidden' }}
              onClick={() => setShowCreateRoom(false)}
              onWheel={(e) => e.stopPropagation()}
            >
              <div
                className="w-full rounded-t-3xl flex flex-col overflow-hidden"
                style={{ height: '72%', background: 'white', marginBottom: 60 }}
                onClick={e => e.stopPropagation()}
              >
                {/* 弹窗标题 */}
                <div className="flex items-center justify-between px-5 py-4 flex-shrink-0" style={{ borderBottom: '1px solid #F0F0F0' }}>
                  <span style={{ fontSize: 18, fontWeight: 700, color: '#1A1A1A' }}>创建新圈子</span>
                  <button
                    onClick={() => setShowCreateRoom(false)}
                  ...
                  >
                    <CloseOutlined style={{ fontSize: 14, color: '#999' }} />
                  </button>
                </div>

                {/* 表单 */}
                <div className="flex-1 overflow-y-auto px-5 py-5 flex flex-col gap-4" style={{ overscrollBehavior: 'contain', minHeight: 0 }}>
                  <div>
                    <div style={{ fontSize: 14, fontWeight: 600, color: '#333', marginBottom: 8 }}>圈子名称</div>
                    <input
                      value={newRoomForm.name}
                      onChange={e => setNewRoomForm(prev => ({ ...prev, name: e.target.value }))}
                      placeholder="给你的圈子起个名字"
                      className="w-full rounded-xl px-4 border-0"
                      style={{ height: 48, background: '#F5F5F5', fontSize: 15, color: '#1A1A1A', outline: 'none' }}
                    />
                  </div>

                  <div>
                    <div style={{ fontSize: 14, fontWeight: 600, color: '#333', marginBottom: 8 }}>圈子简介</div>
                    <textarea
                      value={newRoomForm.description}
                      onChange={e => setNewRoomForm(prev => ({ ...prev, description: e.target.value }))}
                      placeholder="简单介绍一下这个圈子"
                      className="w-full rounded-xl px-4 py-3 border-0 resize-none"
                      style={{ height: 80, background: '#F5F5F5', fontSize: 15, color: '#1A1A1A', outline: 'none' }}
                    />
                  </div>

                  <div>
                    <div style={{ fontSize: 14, fontWeight: 600, color: '#333', marginBottom: 8 }}>选择标签</div>
                    <div className="flex flex-wrap gap-2">
                      {ROOM_TAGS.map(tag => {
                        const isActive = newRoomForm.tag === tag;
                        return (
                          <button
                            key={tag}
                            onClick={() => setNewRoomForm(prev => ({ ...prev, tag: isActive ? '' : tag }))}
                            className="rounded-full border-0 cursor-pointer transition-all active:scale-95"
                            style={{
                              height: 34, padding: '0 16px',
                              background: isActive ? PRIMARY : '#F5F5F5',
                  ...
                  <button
                    onClick={() => {
                      ...
                      const newRoom: ChatRoom = {
                        ...
                        description: newRoomForm.description.trim() || '新的圈子，欢迎加入',
                        tag: newRoomForm.tag || '社交',
                        color: colors[Math.floor(Math.random() * colors.length)],
                        onlineCount: 1,
                        memberCount: 1,
                        lastMessage: '圈子刚建立，快来聊天吧！',
                        lastMessageTime: '刚刚',
                      };
                    ...
                    style={{
                      height: 52,
                      background: newRoomForm.name.trim()
                        ? `linear-gradient(90deg, ${PRIMARY}, #FFAA5A)`
                        : '#E0E0E0',
                      fontSize: 17,
                      boxShadow: newRoomForm.name.trim() ? `0 4px 14px ${PRIMARY}40` : 'none',
                    }}
                  >
                    创建圈子
                  </button>
...
// ─────────────────────────────────────────────
  ...
  const roomChatListRef = useRef<HTMLDivElement>(null);
  ...
  const pushScreen = (next: AppScreen) => {
    if (screen === 'main') {
      ...
      setSavedYishouSubTab(yishouSubTab);
    }
  ...
  const enterRoom = (roomId: string) => {
    ...
    const msgs = ROOM_MESSAGES_MAP[roomId] || [];
    setRoomMessages(msgs);
    pushScreen('room-chat');
  };
    ...
    >
      <div className="min-h-screen flex items-center justify-center py-8" style={{ background: 'linear-gradient(135deg, #FFF4EF 0%, #EBF4FF 100%)' }}>
        <div className="relative flex flex-col select-none overflow-hidden rounded-[48px]"
          ...
          {screen === 'main' && (
            <>
              ...
              <div className="flex-1 overflow-y-auto" style={{ background: activeTab === 'profile' ? 'transparent' : (activeTab === 'yishou' && (yishouSubTab === 'follow' || yishouSubTab === 'plaza')) ? '#000' : PAGE_BG, position: 'relative' }}>
                {activeTab === 'bayu' && <BayuTab onOpenPost={openPost} onOpenVideo={openVideo} onOpenComment={openComment} onOpenUserProfile={openUserProfile} onCreatePost={() => pushScreen('create-select')} onNavigate={pushScreen} yishouSubTab={yishouSubTab} setYishouSubTab={setYishouSubTab} onShare={onShare} userPoints={userPoints} onSetUserPoints={setUserPoints} onEnterRoom={enterRoom} />}
          ...
          {/* ── ROOM CHAT SCREEN ── */}
          {screen === 'room-chat' && activeRoomId && (() => {
            const room = DEFAULT_CHAT_ROOMS.find(r => r.id === activeRoomId);
            if (!room) return null;
            return (
              <RoomChatScreen
                roomName={room.name}
                ...
                onBack={goBack}
                onSend={(text) => setRoomMessages(prev => [...prev, { id: Date.now(), type: 'text' as MsgType, senderName: '', senderAvatar: '', senderColor: '', isMe: true, content: text }])}
              />
            );
          })()}
